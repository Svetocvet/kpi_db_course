...
from controller import Tags


class Neo4jServer(object):
    def __init__(self):
        self.__driver = GraphDatabase.driver("bolt://localhost:7687", auth=("neo4j", "admin"))
    ...
    def users_by_tags(self, tags):
        return self.__record_to_list(self.users_by_tags_neo4j(tags), 'name')

    def by_tags_without_relations(self, tags):
        list_of_names = self.__record_to_list(self.users_by_tags_neo4j(tags), 'name')
        unrelated_users = []
    ...
    def users_by_tags_neo4j(self, tags):
        with self.__driver.session() as session:
            tags = tags.split(", ")
            for tag in tags:
    ...
    def shortest_path(self, username1, username2):
